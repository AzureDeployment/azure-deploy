...
foreach($parentDir in Get-ChildItem -Directory)
{
  foreach($childDir in Get-ChildItem -Path $parentDir -Directory)
  {
    ...
    $temp = "New-AzResourceGroupDeployment -Name {0} -ResourceGroupName {1} -TemplateFile {2} -TemplateParameterFile {3}" -f $childDir, $resourceGroupName, $templateFile, $paramFile
    Write-Information $temp
