  <#
      ...
      .EXAMPLE
      .\deploy-adap-platform -azureEnvironment AzureCloud -orgTag "qui" -location "eastus" -envTag "dev" -deployAction "create" -adapCMDBfile "dev-adap-cmdb.xlsm" -azAll -Force
      .\deploy-adap-platform -azureEnvironment AzureCloud -orgTag "qui" -location "eastus" -envTag "dev" -deployAction "create" -adapCMDBfile "dev-adap-cmdb.xlsm" -removeRG -azAll
      .\deploy-adap-platform -azureEnvironment AzureCloud -orgTag "qui" -location "eastus" -envTag "dev" -deployAction "create" -adapCMDBfile "dev-adap-cmdb.xlsm" -azParameterFiles -azMdFiles
  ...
  # force
    ...
    [validateset('create','remove','purge')]
    [string]$deployAction,
    # onPremAD
  ...
  #$ConfirmPreference = 'None' # 'None','Low','Medium','High' #This is the reason for initiatives, actiongroups and alerts failing.  Leave commented out.
  ...
  if ( (Test-path -Path $adapCMDB) -and (Test-path -Path ('{0}\azure-common.psm1' -f "$psCommonDirectory")))
  {
    try{
      ...
      # Assign Tag Values
      Write-Information -MessageData '  Assigning Tag Values'
      .\tags\assign-tag-values.ps1 -adapCMDB "$adapCMDB" -psConfigDirectory "$psConfigDirectory" -psAlertDirectory "$armAlertDirectory\alerts\" -env $envTag
    ...
    $dataScienceRG = "rg-$orgTag-mlds-$envTag-$suffix"
  ...
  Set-Location -Path "$rootAzuredeploy" | Out-Null
  ...
  $locationDisplayName = $locationName.DisplayName
  Write-Information -MessageData "Pre-Deployment - Updating $locationDescriptionDefault to $locationDisplayName"
  If ($locationDescriptionDefault -ne $locationDisplayName){
    Update-StringInFile -searchStr $locationDescriptionDefault -replaceStr $locationDisplayName -rootDirectory $variablesDirectory -fileExtension 'yml' -CaseSensitive
    Update-StringInFile -searchStr $locationDescriptionDefault -replaceStr $locationDisplayName -rootDirectory $psConfigDirectory -fileExtension 'psm1' -CaseSensitive
  }
  else {
    Write-Information -MessageData "Location are the same. $locationDescriptionDefault to $locationDisplayName - no changes needed."
  }
  ...
  If ($suffixDefault -ne $suffix){
    Update-StringInFile -searchStr $suffixDefault -replaceStr $suffix -rootDirectory $rootAzuredeploy -fileExtension 'json' -ignoreFile 'azure-location-abbr.json'
  ...
  Set-Location -Path "$psscriptsRoot" | Out-Null
  ...
  if($azParameterFiles -or $azAll){
    ...
    Set-Location -Path "$psAzureDirectory" | Out-Null
    .\arm\create-arm-template-parameter-files.ps1 `
        -adapCMDB "$adapCMDB" `
        -paramDirectory "$armTemplatesDirectory\parameters" `
        -env $envTag

    # Assign Tag Values to Alert Parameter Files
    Write-Information -MessageData '  Assigning Tag Values to Alert Parameter Files'
    .\arm\assign-tag-values-parameter-files.ps1 `
        -adapCMDB "$adapCMDB" `
        -psConfigDirectory "$psConfigDirectory" `
        -psAlertDirectory "$armAlertDirectory\alerts\" `
        -env $envTag
  }
  else
  {
  ...
  if($azRoles -or $azAll){
    ...
    Set-Location -Path "$psAzureDirectory" | Out-Null
    .\role\deploy-azure-role-definitions.ps1 `
        -adapCMDB "$adapCMDB" `
        -rootDirectory $armRBACDirectory `
        -subscriptionId $subscriptionId `
        -action $deployAction # purge or create
  }
  else
  {
  ...
  if($azPolicies -or $azAll){
    ...
    Set-Location -Path "$psAzureDirectory" | Out-Null
    .\policy\deploy-azure-policy-definitions.ps1 `
        -adapCMDB "$adapCMDB" `
  ...
  if($azInitiatives -or $azAll){
    ...
    Set-Location -Path "$psAzureDirectory" | Out-Null
    .\policy\deploy-azure-policy-initiatives.ps1 `
        -adapCMDB "$adapCMDB" `
        -rootDirectory "$armPolicyDirectory\initiatives\" `
        -subscriptionId $subscriptionId `
        -action $deployAction `
        -location $location
  }
  ...
  if($azBlueprints -or $azAll){
    ...
    .\blueprint\deploy-azure-blueprint-definitions.ps1 `
        -adapCMDB "$adapCMDB" `
        -rootDirectory "$armBluePrintDirectory" `
        -action $deployAction `
        -subscriptionId $subscriptionId `
        -location "$location" `
  ...
  if($azActionGroups -or $azAll){
    ...
    .\alert\deploy-azure-action-group-defs.ps1 `
        -adapCMDB "$adapCMDB" `
        -rootDirectory "$armAlertDirectory\actiongroup\" `
        -action $deployAction `
        -resourceGroupName $alertResourceGroup
  }
  else
  {
  ...
  if($azAlerts -or $azAll){
    ...
    .\alert\deploy-azure-alert-defs.ps1 `
        -adapCMDB "$adapCMDB" `
