...
Set-Location -Path  $rootDirectory -PassThru | Out-Null
foreach($initFile in Get-ChildItem -Path $rootDirectory\* -Include *.json -Recurse)
{
    try{
      [string]$FullName = $initFile.FullName
      [string]$Name = ($initFile.Name -replace ".json", "")
      $temp = "    Creating Azure Policy Definition:  {0}"  -f $initFile.Name
      Write-Information $temp

      $temp = "New-AzSubscriptionDeployment -Location {0} -TemplateFile {1}" -f $location, $FullName
      Write-Host $temp
      New-AzSubscriptionDeployment -Location $location -TemplateFile $FullName
    }
    catch
    {
