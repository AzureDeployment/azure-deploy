...
$parameters = @{ Env=$env; orgTag=$orgTag; logAnalytics=$logAnalytics; suffix=$suffix; testRG=$testRG; smokeRG=$smokeRG; mgmtRG=$mgmtRG; networkRG=$networkRG; sharedRG=$sharedRG; adapRG=$adapRG ; mldsRG=$dataScienceRG; onpremRG=$onpremRG; location=$location; AzureRegion=$location; Approver = $approver; Owner = $owner; Requester = $requester; BudgetAmount = $budgetAmount; CostCenter = $costCenter; DR = $dr; StartDate = $startDate; EndDate = $endDate; ServiceClass = $serviceClass}

Write-Information "Root Directory: $rootDirectory"
$BPFolders = Get-ChildItem $rootDirectory  

foreach($BPFolder in $BPFolders) {
  $BPName = $BPFolder.Name
  $BPFullName = $BPFolder.FullName
  
  $temp = "    Deploying Azure Blueprint: {0}" -f $BPName
  Write-Information $temp  
  Write-Information "Blueprint Name: $BPName"
  Write-Information "Blueprint FullName: $BPFullName"
  ...
  $temp = "    Imported Artifact: {0}" -f $BPName
  Write-Information $temp
  # success
  if ($?) {
